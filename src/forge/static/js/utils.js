...
function notify(message) {
  if ('Notification' in window) {
    if (Notification.permission === 'granted') {
      var notification = new Notification('Forge', {
        body: message,
        icon: '/static/img/Atlassian-vertical-blue@2x-rgb.png'
      });
...
function createDropdown(parameterKey, defaultValue, dropdownOptions, div) {
  ...
  dropdownAnchor.className =
    'aui-button aui-style-default aui-dropdown2-trigger';
...
function createInputParameter(param, fieldset) {
  ...
  if (param.AllowedValues) {
    ...
    if (
      (action === 'clone' || action === 'create') &&
      (param.ParameterKey === 'DBMasterUserPassword' ||
        param.ParameterKey === 'DBPassword')
    ) {
      ...
      (param.ParameterKey === 'KeyName' ||
        param.ParameterKey === 'KeyPairName') &&
      $('meta[name=ssh_key_name]').attr('value') !== ''
    ) {
      input.value = $('meta[name=ssh_key_name]').attr('value');
    } else if (
      param.ParameterKey === 'HostedZone' &&
      $('meta[name=hosted_zone]').attr('value') !== ''
    ) {
