...
function onReady() {
    ...
    // fix action string
    if (action.indexOf('/') !== -1) action = action.substr(action.indexOf('/') + 1);
    ...
    if (cont) {
        refreshLogsTimer = setTimeout(function() {
            ...
            var logText = $('#log')
                ...
                .toLowerCase();
            if (
                (action === 'diagnostics' && countOccurences(logText, 'dumps complete') >= 1) ||
                countOccurences(
                    logText.replace(/ restart/g, 'restart').replace(/run sql/g, 'runsql'),
                    this_action.toLowerCase() + ' complete'
                ) >= 1
            ) {
