...
function updateStackInfo(stack_name, stack_region) {
  ...
  send_http_get_request(
    baseUrl + "/stackState/" + stack_region + "/" + stack_name,
    displayStackStateAndRequestServiceStatus,
    functionParams
  );
  send_http_get_request(
    baseUrl + "/getActionInProgress/" + stack_region + "/" + stack_name,
    displayActionInProgress
  );
  send_http_get_request(
    baseUrl + "/getVersion/" + stack_region + "/" + stack_name,
    displayVersion
  );
  send_http_get_request(
    baseUrl + "/getNodes/" + stack_region + "/" + stack_name,
    displayNodes
  );
}

function displayStackStateAndRequestServiceStatus(
  responseText,
  functionParams
) {
  $("#stackState").html("Stack status: " + getStatusLozenge(responseText));
  if (
    responseText.trim() === '"CREATE_COMPLETE"' ||
...
function displayActionInProgress(responseText) {
  ...
  $("#currentAction").html(
    "Action in progress: " + getStatusLozenge(actionInProgress, "moved")
  );
  if (
    actionInProgress.toLowerCase() !== "none" &&
...
function getStatusLozenge(text, cssClass) {
  ...
  return (
    '<span class="aui-lozenge aui-lozenge-' + cssClass + '">' + text + "</span>"
  );
}
...
function performAction() {
  ...
  send_http_get_request(
    baseUrl + "/do" + action + "/" + region + "/" + stack_name
  );
