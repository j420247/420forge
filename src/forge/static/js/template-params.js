...
function getTemplates(template_type) {
    send_http_get_request(baseUrl + '/getTemplates/' + template_type, displayTemplates);
}
...
function displayTemplates(responseText) {
    ...
    for (var i = 0; i < templateOptions.length; i++) {
        templateOptions[i].addEventListener(
            'click',
            function(data) {
...
function getTemplateParams(template) {
    ...
    send_http_get_request(
        baseUrl + '/templateParams/' + repo + '/' + template_name,
...
function displayTemplateParams(responseText) {
    ...
    for (var param in origParams) createInputParameter(origParams[param], fieldset);
...
function selectTemplateForStack(stackToRetrieve, templateName) {
    ...
    send_http_get_request(
        baseUrl + '/stackParams/' + region + '/' + stackToRetrieve + '/' + templateName,
        displayStackParams
    );
}
...
function displayStackParams(responseText) {
    ...
    if (action === 'clone') {
        ...
        if (
            document.getElementById('CatalinaOptsVal').value.indexOf(commonMailDisableParams) === -1
        ) {
        ...
        if (product == 'Confluence') {
            if (
                document
                    .getElementById('CatalinaOptsVal')
                    .value.indexOf(confluenceMailDisableParams) === -1
            ) {
...
function getSnapshots(clone_region, stackToRetrieve) {
    ...
    send_http_get_request(
        baseUrl + '/getEbsSnapshots/' + clone_region + '/' + stackToRetrieve,
        displayEbsSnapshots
    );
    send_http_get_request(
        baseUrl + '/getRdsSnapshots/' + clone_region + '/' + stackToRetrieve,
        displayRdsSnapshots
    );
}
...
function displayVPCs(responseText, functionParams) {
    ...
    var default_vpcs = JSON.parse(
        $('meta[name=default_vpcs]')
            .attr('value')
            .replace(/'/g, '')
            .substr(1)
    );
...
function getSubnets(vpc, createOrUpdateList) {
    ...
    var subnets_region = $('#regionSelector').length
        ? $('#regionSelector')
              .text()
              .trim()
        : region;
...
function displaySubnets(responseText, functionParams) {
    ...
    if (functionParams.createOrUpdateList === 'create') {
        createMultiSelect(
            ...
            '',
            subnets,
            document.getElementById('ExternalSubnetsDiv')
        );
        createMultiSelect(
            'InternalSubnets',
            '',
            subnets,
            document.getElementById('InternalSubnetsDiv')
        );
    } else {
...
function selectDefaultSubnets(vpc) {
    ...
    var default_subnets = JSON.parse(
        $('meta[name=default_subnets]')
            .attr('value')
            .replace(/'/g, '')
            .substr(1)
    );
