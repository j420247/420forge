...
function getTemplates(template_type) {
  send_http_get_request(
    baseUrl + '/getTemplates/' + template_type,
    displayTemplates
  );
}
...
function displayTemplates(responseText) {
  ...
  for (var template in templates) {
    ...
    var text = document.createTextNode(
      templates[template][0] + ': ' + templates[template][1]
    );
  ...
  if (action === 'clone') {
    ...
    if (
      document
        ...
        .value.indexOf(commonMailDisableParams) === -1
    ) {
...
function sendParamsAsJson() {
  ...
  for (var i = 0; i < newParams.length; i++) {
    var jsonParam = {};
    var param = newParams.item(i).getElementsByClassName('paramLbl')[0]
      .innerHTML;
    var value;
