...
function checkZDUCompatibility(stack_name) {
    ...
    send_http_get_request(baseUrl + "/getZDUCompatibility/" + region + "/" + stack_name, displayZDUCompatibility);
}
...
function beginUpgrade() {
    if (document.getElementById("zduCheckBox").checked) {
        $("#modal-ok-btn").on("click", performUpgrade);
...
function performUpgrade() {
    ...
    var url = baseUrl + "/doupgrade/" + region + "/" + stack_name + "/" + version + "/" + zdu;
    send_http_post_request(url, JSON.stringify(zdu ? getAuthDetailsAsJSON() : [{}]));
